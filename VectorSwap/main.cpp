...
void vector_swap(vector<int> &v1,vector<int> &v2,
 int start1, int end1,
 int start2, int end2) {
     vector<int> slice1(end1-start1);
     copy(v1.begin()+start1,v1.begin()+end1,slice1.begin());
     vector<int> slice2(end2-start2);
     copy(v2.begin()+start2,v2.begin()+end2,slice2.begin());
     v1.erase(v1.begin()+start1,v1.begin()+end1);
     v2.erase(v2.begin()+start2,v2.begin()+end2);
     for (int num2:slice2){
        v1.insert(v1.begin()+start1,num2);
        start1++;
     }
     for (int num1:slice1){
        v2.insert(v2.begin()+start2,num1);
        start2++;
     }
